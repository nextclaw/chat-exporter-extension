...
- 输出格式可选：popup 在 Export 按钮上方提供 Markdown / JSON 复选框，默认只勾 Markdown；选择持久化到 `chrome.storage.local`。`START_EXPORT` 和 `CHAT_EXPORTER_EXPORT_CURRENT` 携带 `formats: ExportFormat[]`，由 `buildExportBundle` 过滤产出。资产文件无论选什么格式都会下载。
...
- 当前准备发布版本：`0.1.4`（等 `0.1.3` 在 Chrome Web Store 过审后再上传，否则会替换审核排队中的版本）。
