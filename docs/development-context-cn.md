...
- Claude artifact：对话流里只有元数据卡片（`group/artifact-block`：标题 + 类型 + Download 按钮），完整正文在独立侧边面板、不在对话 DOM 里。要拿正文必须点击（开面板 / Download），违反不变量，因此**不捕获正文**，只识别卡片渲染成 `[Attachment: <标题> (<类型>)]` 占位（标题取覆盖层 `aria-label="View/Download ..."`，最稳）。明确决定：不做模拟点击 Download 的下载功能。
...
- 弹窗预览：popup 启动时通过 `probeCurrentPageSummary` 取得标题 + 轮数（按 user 消息计数，重试的 assistant 仍算同一轮，与导出文件 frontmatter 的 `turns:` 字段一致），状态行显示 `Ready: <title> · N turns`。ChatGPT 虚拟列表在导出前可能未挂载全部 turn（placeholder 数 > 已挂载消息数），此时不显示轮数而非显示偏小的错值；Gemini / Claude 非虚拟化，正常显示。
