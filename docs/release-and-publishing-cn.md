...
- 当前准备发布版本：`0.1.4`（先等 `0.1.3` 过审，再上传 `0.1.4`，避免覆盖正在排队的审核版本）
...
```text
release/chat-exporter-extension-v0.1.4.zip
```

检查这个 ZIP 是否能被 Chrome Web Store 接受的关键点：

```bash
unzip -l release/chat-exporter-extension-v0.1.4.zip | sed -n '1,60p'
```
...
5. 解压下载到本地的 artifact
6. 找到里面的 `chat-exporter-extension-v0.1.4.zip`

注意：GitHub 下载的 artifact 本身通常也是一个 ZIP。Chrome Web Store 要上传的是 artifact 里面的扩展 ZIP，也就是 `chat-exporter-extension-v0.1.4.zip`。
...
```text
release-artifacts/chat-exporter-extension-v0.1.4.zip
```
...
```text
/opt/local/src/nextclaw/chat-exporter-extension/release/chat-exporter-extension-v0.1.4.zip
```
...
2. 点击 `Add new item`
3. 选择 `chat-exporter-extension-v0.1.4.zip`
4. 点击上传
...
这是正常的。GitHub artifact 下载下来可能是外层 ZIP。先解压外层 ZIP，里面的 `chat-exporter-extension-v0.1.4.zip` 才是 Chrome Web Store 上传包。
...
```bash
unzip -l chat-exporter-extension-v0.1.4.zip | sed -n '1,60p'
```
