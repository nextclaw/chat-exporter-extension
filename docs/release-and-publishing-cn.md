...
- 当前准备发布版本：`0.1.6`（按 `0.1.3` → `0.1.4` → `0.1.5` → `0.1.6` 顺序上传，每版都等前一版过审后再传，避免覆盖审核队列）

## 0.1.6 发布说明

- **Claude artifact 作为附件标注**：识别真实的 `group/artifact-block` 卡片（旧选择器匹配不到），渲染成干净的 `[Attachment: <标题> (<类型>)]` 占位，不再把卡片的标题/类型/Download 按钮文字漏进正文。artifact 完整正文在侧边面板、不在对话 DOM，强拿需点击（违反不变量），故只标注不捕获。明确不做模拟点击下载。
- 不含其他功能或权限变更。
...
```text
release/chat-exporter-extension-v0.1.6.zip
```

检查这个 ZIP 是否能被 Chrome Web Store 接受的关键点：

```bash
unzip -l release/chat-exporter-extension-v0.1.6.zip | sed -n '1,60p'
```
...
5. 解压下载到本地的 artifact
6. 找到里面的 `chat-exporter-extension-v0.1.6.zip`

注意：GitHub 下载的 artifact 本身通常也是一个 ZIP。Chrome Web Store 要上传的是 artifact 里面的扩展 ZIP，也就是 `chat-exporter-extension-v0.1.6.zip`。
...
```text
release-artifacts/chat-exporter-extension-v0.1.6.zip
```
...
```text
/opt/local/src/nextclaw/chat-exporter-extension/release/chat-exporter-extension-v0.1.6.zip
```
...
2. 点击 `Add new item`
3. 选择 `chat-exporter-extension-v0.1.6.zip`
4. 点击上传
...
这是正常的。GitHub artifact 下载下来可能是外层 ZIP。先解压外层 ZIP，里面的 `chat-exporter-extension-v0.1.6.zip` 才是 Chrome Web Store 上传包。
...
```bash
unzip -l chat-exporter-extension-v0.1.6.zip | sed -n '1,60p'
```
